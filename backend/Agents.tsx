  ...
  // Clean Agent interface matching realTimeAgent.js format
  interface Agent {
    id: string;
    extension: string;
    first_name: string;
  ...
  const Agents: React.FC = () => {
    ...
    const mapAgentStatusForDisplay = (deviceState: string, status: string): string => {
    ...
    useEffect(() => {
      ...
      // Listen only to the clean format from realTimeAgent.js
    ...
    const agentSummary = React.useMemo(() => {
      ...
      agents.forEach(agent => {
        const displayStatus = mapAgentStatusForDisplay(agent.deviceState, agent.status);
        ...
        // Add logic for away/paused based on liveStatus if needed
        else if (agent.liveStatus === 'Paused') summary.paused++;
        else if (agent.liveStatus === 'Away') summary.away++;
      });
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-600 mt-1">Monitor agent status and performance in real-time</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(displayStatus)}`}
                        >
                          {displayStatus}
                        </span>
                      </div>
                      ...
                      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <p className="text-sm text-gray-600">Live Status: <span className="font-medium">{agent.liveStatus}</span></p>
                        {agent.contacts && (
                          <p className="text-xs text-gray-500 mt-1">Contact: {agent.contacts}</p>
                        )}
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-green-600">{stats.totalCalls}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-red-600">{stats.missedCalls}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-blue-600">{formatTime(stats.averageTalkTime)}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-yellow-600">{formatTime(stats.longestIdleTime)}</span>
                        </div>
                        ...
                        <div className="pt-2 border-t border-gray-200">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-semibold text-indigo-600">
                              {stats.totalCalls > 0 
                                ? `${(((stats.totalCalls - stats.missedCalls) / stats.totalCalls) * 100).toFixed(1)}%`
                                : '0%'
                              }
                            </span>
                          </div>
                    ...
                    {paginatedAgents.map((agent) => {
                      ...
                      return (
                        <tr key={agent.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(displayStatus)}`}
                            >
                              {displayStatus}
                            </span>
                            <div className="text-xs text-gray-500 mt-1">{agent.liveStatus}</div>
                          </td>
