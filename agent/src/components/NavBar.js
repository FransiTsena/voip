...
const NavBar = ({ onLogout, isSIPReady, agentStatus, setAgentStatus }) => {
    ...
    const fetchShiftReport = async () => {
        ...
        try {
            const res = await fetch(`/api/metrics/agent/${user._id}/shifts`);
                ...
                <div className="flex items-center gap-8">
                    ...
                    {user && (
                        <div className="flex flex-col items-end mr-4">
                            <span className="text-blue-500 font-bold text-lg flex items-center gap-2"><UserCog className="text-blue-200" /> {user.displayName}</span>
