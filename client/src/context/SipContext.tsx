...
interface SipContextType {
    ua: JsSIP.UA | null;
    status: string;
    incomingSession: JsSIP.RTCSession | null;
...
interface SipProviderProps { children: ReactNode; config: { wsUri: string; sipUri: string; password: string; displayName?: string }; }

export const SipProvider: React.FC<SipProviderProps> = ({ children, config }) => {
    const uaRef = useRef<JsSIP.UA | null>(null);
    const [status, setStatus] = useState<string>('disconnected');
    const [incomingSession, setIncomingSession] = useState<JsSIP.RTCSession | null>(null);
    const [incomingNumber, setIncomingNumber] = useState<string | null>(null);

    const cleanSession = (session: JsSIP.RTCSession) => {
    ...
    useEffect(() => {
        ...
        ua.on('newRTCSession', (data: any) => {
            ...
            if (data.originator === 'remote') {
                ...
                setIncomingNumber((session as any).remote_identity?.uri?.user || (session as any).remote_identity?.uri || 'Unknown');
            }
        });
    ...
    const decline = useCallback(() => {
        if (incomingSession) {
            try { incomingSession.terminate(); } catch { /* ignore */ }
            setIncomingSession(null); setIncomingNumber(null);
        }
    }, [incomingSession]);

    return (
        <SipContext.Provider value={{ ua: uaRef.current, status, incomingSession, incomingNumber, answer, decline }}>
            {children}
        </SipContext.Provider>
    );
};
