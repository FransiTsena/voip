...
const AgentList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      ...
      setError("");
      try {
        const res = await axios.get(`${API}/api/agent`, { withCredentials: true });
        setAgents(res.data);
      } catch (err: any) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`${API}/api/agent/${agentToDelete._id}`, { withCredentials: true });
                ...
                <tr key={agent._id} className="hover:bg-blue-50 transition-colors">
                  ...
                  <td className="py-3 px-4 border-b">
                    <button
                      ...
                      onClick={() => navigate(`/users/edit/${agent._id}`)}
                      title="Edit"
                    >
                      <FiEdit size={18} />
                    </button>
